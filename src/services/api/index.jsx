...
requestSHNK.interceptors.request.use(
  (config) => {
...
requestSHNK.interceptors.response.use(
  (response) => {
    return response;
  },
  (error) => {
...
requestTMSITI.interceptors.request.use(
  (config) => {
    const token = get(JSON.parse(storage.get("settings")), "state.token", null);
    if (token) {
      config.headers["token"] = `${token}`;
...
requestTMSITI.interceptors.response.use(
  (response) => {
    return response;
  },
  (error) => {
